...
def save_github_stars(starred_repos: list[dict]) -> list[stars_domain.GithubStarredRepositoryModel]:
    ...
    with session_pool() as session:
        ...
        for repo_data in starred_repos:
            ...
            ## Create owner model
            repo_owner_schema: stars_domain.GithubRepositoryOwnerIn = stars_domain.GithubRepositoryOwnerIn.model_validate(repo_owner_data)
            repo_owner: stars_domain.GithubRepositoryOwnerModel = stars_domain.converters.convert_github_repository_owner_schema_to_db_model(owner=repo_owner_schema)
            
            ## Create Repository Model instance
            github_repo_schema: stars_domain.GithubStarredRepoIn = stars_domain.GithubStarredRepoIn.model_validate(repo_data)
            github_repo: stars_domain.GithubStarredRepositoryModel = stars_domain.converters.convert_github_starred_repo_schema_to_db_model(github_repo_schema)
