...
class GithubStarredRepositoryDBRepository(db_lib.base.BaseRepository[GithubStarredRepositoryModel]):
    def __init__(self, session: so.Session):
    ...
    def create_or_get_repo(self, github_repo: GithubStarredRepositoryModel, repo_owner: GithubRepositoryOwnerModel) -> GithubStarredRepositoryModel:
        ...
        try:
            ...
            existing_repo: GithubStarredRepositoryModel | None = self.session.query(
                GithubStarredRepositoryModel
            ...
            existing_repo_owner: GithubRepositoryOwnerModel | None = self.session.query(
                GithubRepositoryOwnerModel
            ).filter(GithubRepositoryOwnerModel.id == repo_owner.id).one_or_none()

            if existing_repo_owner:
                log.info(f"Existing owner found with owner_id '{repo_owner.id}'. Checking if repository '{github_repo.repo_id}' is already linked to this owner")
                ...
                for repo in existing_repo_owner.repositories:
                    if repo.repo_id == github_repo.repo_id:
                        log.info(f"Repository '{github_repo.repo_id}' is already linked to owner '{repo_owner.id}'. Returning existing repository.")
                        return repo  # Return if the repo is already linked

                ## Repository does not exist, add it to the owner's repositories
                log.info(f"Repository '{github_repo.repo_id}' not found under owner '{repo_owner.id}', linking repository to owner.")
                ...
                log.info(f"Owner '{repo_owner.id}' not found, creating new owner and linking repository.")
