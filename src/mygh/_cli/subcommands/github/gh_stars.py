...
@gh_stars_app.command(name="get", help="Get starred repositories associated with Github PAT.")
def get_user_stars(
    ...
    if api_token is None:
        ...
        if api_token is None:
            raise ValueError("Missing a Github PAT to use with the API. Please set a value in your config/.secrets.local.toml, or set the GH_API_TOKEN environment variable.")
        
    try:
        starred_repos: list[dict] = client.get_starred_repos(api_token=api_token, use_cache=use_cache, cache_ttl=cache_ttl)
    ...
    if save:
        ...
        try:
            saved_stars = client.save_github_stars(starred_repos=starred_repos)
