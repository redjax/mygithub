...
def validate_trim(trim_in: int = 0, as_hex: bool = False) -> int:
    ...
    if as_hex:
        ...
        _uuid_len: int = glob_uuid_lens.standard

    if not trim_in >= 0:
        raise ValueError(f"Trim value must be 0 or greater.")

    ## Check that trim_in does not exceed length of UUID string
    if trim_in >= _uuid_len:
        exc_msg: str = f"Trim value must be less than {_uuid_len}. At least 1 character must be returned."

        if as_hex:
            exc_msg: str = f"{exc_msg} Note that a hexadecimal UUID string is only 32 characters because of the missing '-' characters." # type: ignore
...
def validate_characters(characters_in: int = 0, as_hex: bool = False) -> int:
    ...
    if as_hex:
        ...
        if characters_in > _uuid_len:
            raise ValueError(
                f"Character count must be less than UUID string length ({_uuid_len})"
            )
    ...
    if characters_in >= _uuid_len:
        ...
        if as_hex:
            exc_msg: str = f"{exc_msg} Note that a hexadecimal UUID string is only {glob_uuid_lens.hex} characters because of the missing '-' characters." # type: ignore
