...
def ensure_dir_exists(path: str) -> None:
    ...
    if not _path.exists():
        ...
        try:
            ...
            msg = f"({type(exc)}) Error creating directory '{_path}'. Details: {exc}"
            log.error(msg)
...
def get_http_controller(
    ...
    cache_type: str | None = HTTP_SETTINGS.get("HTTP_CACHE_TYPE", default="sqlite"),
    cache_file_dir: str | None = HTTP_SETTINGS.get(
        "HTTP_CACHE_FILE_DIR", default=".cache/http/hishel"
    ),
    cache_db_file: str | None = HTTP_SETTINGS.get(
    ...
    try:
        http_ctl: HttpxController = HttpxController(
            ...
            cache_db_file=cache_db_file, # type: ignore
            cache_ttl=cache_ttl,
...
def merge_headers(header_dicts: list[t.Union[str, dict]] | None = []) -> dict:
    ...
    for header_dict in header_dicts:
        ...
        if isinstance(header_dict, str):
            _header_dict: dict = json.loads(header_dict)
            headers = {**headers, **_header_dict}
...
class HttpxController(AbstractContextManager):
    ...
    def __enter__(self) -> t.Self:
        if self.use_cache:
            ...
            transport: hishel.CacheTransport| None = self._get_cache_transport()
            controller: hishel.Controller | None= self._get_cache_controller()
        else:
        ...
        self.client: httpx.Client = self._get_client() # type: ignore
    ...
    def _get_cache(self) -> t.Union[hishel.SQLiteStorage, hishel.FileStorage] | None:
        ...
        match self.cache_type:
            ...
            case "sqlite":
                ...
                sqlite_cache: hishel.SQLiteStorage = cache.get_sqlite_cache_storage(
            ...
            case "file":
                ...
                file_cache: hishel.FileStorage = cache.get_file_cache_storage(
                    base_path=self.cache_file_dir, # type: ignore
                    ttl=self.cache_ttl, # type: ignore
                    check_ttl_every=self.check_ttl_every, # type: ignore
                )
    ...
    def _get_cache_controller(self) -> hishel.Controller | None:
    ...
    def _get_cache_transport(self) -> hishel.CacheTransport | None:
        ...
        if self.cache_controller is None:
            if self.use_cache:
                cache_controller: hishel.Controller | None = self._get_cache_controller()
                self.cache_controller = cache_controller

        if self.use_cache:
            _transport: hishel.CacheTransport = cache.get_cache_transport(
                cache_storage=self.cache, # type: ignore
                cache_controller=self.cache_controller # type: ignore
            )
        else:
            _transport = None
